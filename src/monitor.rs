...
pub async fn run(
    ...
    loop {
        ...
        .await;
        sleep(Duration::from_secs(config.monitors.poll_interval_secs.max(1))).await;
    }
}
...
async fn poll_git(
    ...
    for repo in &config.monitors.git.repos {
        match snapshot_git_repo(repo).await {
            Ok(snapshot) => {
                ...
                if let Some(previous) = previous {
                    if repo.emit_branch_changes && previous.branch != snapshot.branch {
                        ...
                        if let Err(error) = dispatch_event(router, discord, &event, repo.mention.as_deref()).await {
                ...
                let prs = if repo.emit_pr_status {
                    if let Some(client) = github_client {
                        match fetch_pull_requests(client, &config.monitors.github_api_base, repo, &snapshot).await {
                                        ...
                                        match previous.prs.get(number) {
                                            ...
                                            old => {
                                                let event = IncomingEvent::git_pr_status_changed(
                                                    ...
                                                    *number,
                                                    pr.title.clone(),
                                                    old.map(|value| value.status.clone()).unwrap_or_else(|| "<new>".to_string()),
                                                    pr.status.clone(),
                                                    pr.url.clone(),
                                                    repo.channel.clone(),
                                                )
                                                .with_format(repo.format.clone());
                                                if let Err(error) = dispatch_event(router, discord, &event, repo.mention.as_deref()).await {
                                                    eprintln!("clawhip monitor PR dispatch failed: {error}");
                                                }
                                            }
                                        }
                                    }
                            ...
                            Err(error) => {
                                eprintln!("clawhip monitor GitHub polling failed for {}: {error}", repo.path);
            ...
            Err(error) => eprintln!("clawhip monitor git snapshot failed for {}: {error}", repo.path),
        }
    }
}
...
async fn poll_tmux(
    ...
    let mut sessions: BTreeMap<String, RegisteredTmuxSession> = config
        ...
        .sessions
        .iter()
        .map(|session| (session.session.clone(), RegisteredTmuxSession::from(session)))
        .collect();
    ...
    for (session_name, registration) in sessions {
        match session_exists(&session_name).await {
            ...
            Err(error) => {
                eprintln!("clawhip monitor tmux has-session failed for {}: {error}", session_name);
                continue;
            }
            Ok(true) => {}
        }
        ...
        match snapshot_tmux_session(&session_name).await {
                    ...
                    match state.get_mut(&pane_key) {
                        ...
                        Some(existing) => {
                            if existing.content_hash != hash {
                                let hits = collect_keyword_hits(&existing.snapshot, &pane.content, &registration.keywords);
                                for hit in hits {
                                    ...
                                    if let Err(error) = dispatch_event(router, discord, &event, registration.mention.as_deref()).await {
                                ...
                                let stale_after = Duration::from_secs(registration.stale_minutes.max(1) * 60);
                                let should_notify = now.duration_since(existing.last_change) >= stale_after
                                    && existing
                                ...
                                if should_notify {
                                    ...
                                    .with_format(registration.format.clone());
                                    if let Err(error) = dispatch_event(router, discord, &event, registration.mention.as_deref()).await {
                                        eprintln!("clawhip monitor tmux stale dispatch failed: {error}");
                                    }
            ...
            Err(error) => eprintln!("clawhip monitor tmux snapshot failed for {}: {error}", session_name),
        }
    }
...
async fn snapshot_git_repo(repo: &GitRepoMonitor) -> Result<GitSnapshot> {
    ...
    let branch = run_command(&git_bin(), &["-C", &repo.path, "rev-parse", "--abbrev-ref", "HEAD"]).await?;
    ...
    Ok(GitSnapshot {
        repo_name: repo
            .name
            .clone()
            .unwrap_or_else(|| Path::new(&repo.path).file_name().and_then(|name| name.to_str()).unwrap_or(&repo.path).to_string()),
        branch,
        head: head.clone(),
        commits: vec![CommitEntry { sha: head, summary }],
        github_repo: repo.github_repo.clone().or_else(|| parse_github_repo(&remote_url)),
    })
}
...
async fn list_new_commits(repo: &GitRepoMonitor, old: &str, new: &str) -> Result<Vec<CommitEntry>> {
    ...
    Ok(output
        ...
        .filter_map(|line| {
            ...
            Some(CommitEntry { sha: sha.to_string(), summary: summary.to_string() })
        })
        .collect())
}
...
async fn fetch_pull_requests(
    ...
    let response = client
        .get(format!("{}/repos/{}/pulls", api_base.trim_end_matches('/'), github_repo))
    ...
    Ok(pulls
        .into_iter()
        .map(|pull| {
            let status = if pull.merged_at.is_some() { "merged".to_string() } else { pull.state };
            (
                pull.number,
                PullRequestSnapshot { title: pull.title, status, url: pull.html_url },
            )
        })
        .collect())
}
...
async fn session_exists(session: &str) -> Result<bool> {
    let output = Command::new(tmux_bin()).arg("has-session").arg("-t").arg(session).output().await?;
...
async fn snapshot_tmux_session(session: &str) -> Result<Vec<TmuxPaneSnapshot>> {
    ...
    for line in String::from_utf8(output.stdout)?.lines() {
        ...
        if !capture.status.success() {
            return Err(String::from_utf8_lossy(&capture.stderr).trim().to_string().into());
        }
...
async fn run_command(binary: &str, args: &[&str]) -> Result<String> {
    ...
    if output.status.success() {
        ...
        Err(format!("{} {:?} failed: {}", binary, args, String::from_utf8_lossy(&output.stderr).trim()).into())
    }
}
    ...
    current
        ...
        .flat_map(|line| {
            keywords.iter().filter_map(move |keyword| {
                if line.to_ascii_lowercase().contains(&keyword.to_ascii_lowercase()) {
                    Some(KeywordHit { keyword: keyword.clone(), line: line.to_string() })
                } else {
                    None
                }
            })
...
mod tests {
    ...
    fn parses_github_repo_urls() {
        assert_eq!(parse_github_repo("git@github.com:bellman/clawhip.git"), Some("bellman/clawhip".to_string()));
        assert_eq!(parse_github_repo("https://github.com/bellman/clawhip.git"), Some("bellman/clawhip".to_string()));
    }
